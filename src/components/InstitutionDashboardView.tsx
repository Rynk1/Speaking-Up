  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden min-h-[680px] flex flex-col md:flex-row text-slate-100">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3 px-1">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-amber-500 to-amber-700 flex items-center justify-center shadow-lg text-slate-950 font-black">
      ...
      <main className="flex-1 p-4 sm:p-6 bg-slate-900/60 overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-slate-950/60 border border-slate-800 p-4 rounded-xl space-y-1">
                ...
                <div className="text-[10px] text-slate-500">Citizen mentions across regions</div>
              </div>

              <div className="bg-slate-950/60 border border-amber-900/60 p-4 rounded-xl space-y-1">
                ...
                <div className="text-[10px] text-slate-500">Requires immediate dispatch</div>
              </div>

              <div className="bg-slate-950/60 border border-red-900/60 p-4 rounded-xl space-y-1">
              ...
              <div className="bg-slate-950/60 border border-emerald-900/60 p-4 rounded-xl space-y-1">
                ...
                <div className="text-[10px] text-slate-500">Public accountability statements</div>
              </div>
            </div>
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 sm:p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-sm text-white flex items-center gap-2">
                  <Flame className="w-4 h-4 text-amber-400" /> Recent High-Priority Citizen Alerts Requiring Response
                </h3>
                <button
                  onClick={() => setActiveTab('alerts')}
              ...
              <div className="space-y-3">
                {unansweredPosts.slice(0, 3).length === 0 ? (
                  ...
                  unansweredPosts.slice(0, 3).map((post, idx) => (
                    <div key={post.id} className="p-3 bg-slate-900/80 border border-slate-800 rounded-xl space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-white">{post.title}</span>
                        <span className="text-[10px] text-slate-400">{post.location.district} ({post.location.region})</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 text-xs">
                        ...
                        <button
                          ...
                          className="px-3 py-1 bg-amber-600 hover:bg-amber-500 text-slate-950 text-xs font-bold rounded-lg"
                        >
                          Respond Officially
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800 flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            {hasResponded ? 'Update Public Statement' : 'Respond Officially'}
                          </button>
                        </div>
                      </div>
                    </div>
        ...
        {activeTab === 'urgent' && (
          <div className="space-y-4">
            <div className="p-4 bg-red-950/40 border border-red-800/60 rounded-xl text-xs text-red-200 space-y-1">
            ...
            <div className="space-y-3">
              {criticalPosts.length === 0 ? (
                ...
                criticalPosts.map((post, idx) => (
                  <div key={post.id} className="bg-slate-950/60 border border-red-900/50 rounded-2xl p-4 sm:p-5 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <span className="text-[10px] font-bold px-2 py-0.5 bg-red-500/20 text-red-400 border border-red-500/30 rounded uppercase">
                          {post.urgency} THREAT
                        </span>
                        <h3 className="font-bold text-base text-white mt-1">{post.title}</h3>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
                      <button
                        onClick={() => onOpenResponseModal(post)}
                        className="px-4 py-1.5 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-md"
                      >
        ...
        {activeTab === 'responses' && (
          <div className="space-y-4">
            <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl text-xs text-slate-300 space-y-1">
            ...
            <div className="space-y-3">
              {answeredPosts.length === 0 ? (
                ...
                answeredPosts.map((post, idx) => {
                  ...
                  return (
                    <div key={resp.id || idx} className="bg-slate-950/60 border border-emerald-900/50 rounded-2xl p-4 sm:p-5 space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <span className="text-xs font-bold text-emerald-400 flex items-center gap-1">
                            <ShieldCheck className="w-3.5 h-3.5" /> Official Statement Live
                          </span>
                          <h4 className="font-bold text-sm text-white mt-1">Re: {post.title}</h4>
                        </div>
                        <span className="text-[11px] text-slate-400">{new Date(resp.createdAt).toLocaleDateString()}</span>
                      </div>

                      <div className="bg-slate-900/80 p-3 rounded-xl border border-slate-800 text-xs text-slate-200 leading-relaxed">
                        <div className="text-[10px] text-slate-400 font-bold mb-1">
                          {resp.responderName} ({resp.responderTitle})
                        </div>
                        "{resp.message}"
                      </div>
                        ...
                        <div className="flex items-center gap-2">
                          {onViewResponseFeedPost && (
                            <button
                              ...
                              className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-[11px] font-semibold"
                            >
                              Feed Post View
                            </button>
                          )}
                          {onViewOfficialResponse && (
                            <button
                              ...
                              className="px-3 py-1 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 rounded-lg text-[11px] font-semibold"
                            >
                              Statement Thread →
                            </button>
                          )}
            ...
            <div className="p-5 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-3 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
                  <div className="text-slate-400 text-[10px]">Official Name</div>
                  <div className="font-bold text-white text-sm">{currentInstitution?.officialName}</div>
                </div>

                <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
                  <div className="text-slate-400 text-[10px]">Short Name & Acronym</div>
                  <div className="font-bold text-white text-sm">{currentInstitution?.shortName} ({currentInstitution?.acronym})</div>
                </div>

                <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
                  <div className="text-slate-400 text-[10px]">Category & Jurisdiction</div>
                  <div className="font-bold text-emerald-400">{currentInstitution?.category} — {currentInstitution?.jurisdiction}</div>
                </div>

                <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
                  <div className="text-slate-400 text-[10px]">Alert Dispatch Method</div>
                  <div className="font-bold text-amber-300 font-mono">{currentInstitution?.alertMethod || 'OFFICIAL_EMAIL'}</div>
                </div>
              </div>

              <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
                <div className="text-slate-400 text-[10px]">Institutional Mandate</div>
                <div className="text-slate-300">{currentInstitution?.mandate || 'Official Ghanaian public service body.'}</div>
              </div>
            </div>
          </div>
        )}
