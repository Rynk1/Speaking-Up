          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="relative" ref={roleRef}>
              ...
              >
                ...
                {(userRole === 'moderator' || userRole === 'admin') && <Shield className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />}
                <span className="hidden sm:inline capitalize">
                  {userRole === 'citizen' ? 'Citizen' : userRole === 'institution_rep' ? 'Official' : userRole === 'journalist' ? 'Media' : userRole === 'admin' ? 'Admin' : 'Safety'}
                </span>
              ...
              {showRoleMenu && (
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl p-2 z-50 animate-in fade-in zoom-in-95">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-2 rounded-lg text-xs flex items-center justify-between mt-1 ${
                      userRole === 'admin' ? 'bg-purple-100 dark:bg-purple-950/60 text-purple-900 dark:text-purple-300 border border-purple-300 dark:border-purple-800/60 font-semibold' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Shield className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                      <div>
                        <div className="font-semibold">System Admin Dashboard</div>
                        <div className="text-[10px] text-slate-500 dark:text-slate-400">Full website, user & content control</div>
                      </div>
                    </div>
                    {userRole === 'admin' && <CheckCircle2 className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />}
                  </button>
                </div>
              )}
            </div>
