    ...
    <header className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 text-white shadow-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-6">
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-4">
          {/* Logo & Brand */}
          <div className="flex items-center gap-3 cursor-pointer flex-shrink-0" onClick={() => setCurrentView('feed')}>
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold tracking-tight text-base sm:text-lg text-white">
                  GHANA<span className="text-emerald-400">CIVIC</span>
                </span>
                <span className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.2 text-[9px] font-bold bg-amber-500/20 text-amber-300 rounded border border-amber-500/30">
          ...
          <div className="flex-1 max-w-md hidden md:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-1.5 bg-slate-800/90 text-sm text-slate-100 placeholder-slate-400 rounded-lg border border-slate-700 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all"
              />
              {searchQuery && (
                <button
          ...
          {/* Action CTAs: Role Switcher, Notifications, Speak Up Button */}
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={roleRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700/80 border border-slate-700 rounded-lg text-xs text-slate-200 transition-colors"
                title="Switch Viewing Perspective"
              >
                {userRole === 'citizen' && <User className="w-3.5 h-3.5 text-emerald-400" />}
                {userRole === 'institution_rep' && <Building2 className="w-3.5 h-3.5 text-amber-400" />}
                {userRole === 'journalist' && <Newspaper className="w-3.5 h-3.5 text-sky-400" />}
                {userRole === 'moderator' && <Shield className="w-3.5 h-3.5 text-purple-400" />}
              ...
              {showRoleMenu && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-xl p-2 z-50 animate-in fade-in zoom-in-95">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-2 rounded-lg text-xs flex items-center justify-between ${
                      userRole === 'citizen' ? 'bg-emerald-950/60 text-emerald-300 border border-emerald-800/60' : 'text-slate-200 hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">Post & amplify local observations</div>
                      </div>
                    </div>
                    {userRole === 'citizen' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />}
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-2 rounded-lg text-xs flex items-center justify-between mt-1 ${
                      userRole === 'institution_rep' ? 'bg-amber-950/60 text-amber-300 border border-amber-800/60' : 'text-slate-200 hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">Monitor alerts & respond publicly</div>
                      </div>
                    </div>
                    {userRole === 'institution_rep' && <CheckCircle2 className="w-3.5 h-3.5 text-amber-400" />}
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-2 rounded-lg text-xs flex items-center justify-between mt-1 ${
                      userRole === 'journalist' ? 'bg-sky-950/60 text-sky-300 border border-sky-800/60' : 'text-slate-200 hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">Discover breaking public interest stories</div>
                      </div>
                    </div>
                    {userRole === 'journalist' && <CheckCircle2 className="w-3.5 h-3.5 text-sky-400" />}
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-2 rounded-lg text-xs flex items-center justify-between mt-1 ${
                      userRole === 'moderator' ? 'bg-purple-950/60 text-purple-300 border border-purple-800/60' : 'text-slate-200 hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">Prevent mob targeting & review reports</div>
                      </div>
                    </div>
                    {userRole === 'moderator' && <CheckCircle2 className="w-3.5 h-3.5 text-purple-400" />}
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="p-2 text-slate-300 hover:text-white bg-slate-800/80 hover:bg-slate-700/80 rounded-lg border border-slate-700 relative transition-colors"
                      ...
                      notifications.map(n => (
                        <div
                          ...
                          className={`py-2.5 px-2 rounded-lg cursor-pointer transition-colors ${
                            n.read ? 'opacity-60 hover:opacity-100' : 'bg-slate-800/50 hover:bg-slate-800'
                          }`}
                        >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs sm:text-sm font-bold rounded-lg shadow-md hover:shadow-emerald-900/40 transition-all active:scale-95"
            >
